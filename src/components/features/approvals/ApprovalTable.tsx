...
import { Check, X, ExternalLink, Folder } from "lucide-react";
...
interface ApprovalTableProps {
  programName: string;
  items: { task: { id: string; name: string }; program: { type: string }; aos: string[] }[];
  pending: PendingItem[];
  onApprove: (taskId: string, userId: string) => void;
  onReject: (taskId: string, userId: string, aoName: string) => void;
}

export function ApprovalTable({ programName, items, pending, onApprove, onReject }: ApprovalTableProps) {
  return (
    <div className="mb-5">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.flatMap(({ task, program, aos }) =>
              aos.map((userId, i) => {
                const p = pending.find((x) => x.taskId === task.id && x.userId === userId);
                return (
                  <tr key={`${task.id}-${userId}`} className="border-b border-border hover:bg-secondary/30 transition-colors">
                    <td className="p-2.5">
                      {i === 0 && <><span className="font-semibold">{task.name}</span><br /><span className="text-xs text-muted-foreground"><VariantBadge variant={program.type === "Akademik" ? "green" : "blue"}>{program.type}</VariantBadge></span></>}
                    </td>
                    <td className="p-2.5 whitespace-nowrap">
                      <span className="inline-block w-2 h-2 rounded-full mr-1.5" style={{ background: getAOColor(userId) }} />
                    ...
                    <td className="p-2.5 text-center whitespace-nowrap">
                      ...
                      <Button size="sm" variant="ghost" className="text-green-600" onClick={() => onApprove(task.id, userId)}><Check className="w-3.5 h-3.5" /></Button>
                      <Button size="sm" variant="ghost" className="text-red-600" onClick={() => onReject(task.id, userId, userId)}><X className="w-3.5 h-3.5" /></Button>
                    </td>
                  </tr>
                );
              })
